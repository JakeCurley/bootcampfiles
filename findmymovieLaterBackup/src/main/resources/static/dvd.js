  ...
  $("#stars").emojiRating(
    (options = {
      ...
      onUpdate: function (rating) {
        console.log(rating);
        var test = $("input[name=rating]").val();
        console.log(test);
      },
    })
  );
...
function createNewList(name) {
  ...
  $.ajax({
    ...
    success: function (message) {
      ...
      $(".hiddenMovieList").val(name).prop("selected", true);
      $("#newListSuccess").text("");
...
function getCompleteLists() {
  var contentRows = $("#myListContainer");
  $.ajax({
    type: "GET",
    url: "http://localhost:8080/getCompleteLists",
    success: function (movieListArray) {
      $.each(movieListArray, function (index, movie) {
        var listName = movie.listName;
        ...
        row += '<div class="row"><h3>' + listName + "</h5>";
        row +=
          '<div class="col-6"><h5>' + movieTitle + " " + releaseDate + "</h5>";
        row += '<div class="col-6"><h5>' + rating + "</h5>";
        row += "</div>";
